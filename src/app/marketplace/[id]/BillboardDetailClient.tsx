...
function ScoreRing({
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <span
        className="text-lg font-bold -mt-12 relative z-10"
        style={{ color }}
      >
        {value}
      </span>
...
export default function BillboardDetailClient({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "traffic" | "campaigns" | "market"
  >("overview");
    ...
    const loadTraffic = async () => {
      try {
        const res = await fetch(
          `/api/traffic-estimate?lat=${bb.lat}&lng=${bb.lng}`,
        );
        if (!res.ok) return;
        const data = await res.json();
        if (active) {
          setTrafficEstimate({
            estimatedDailyImpressions: data.estimatedDailyImpressions,
            trafficScore: data.trafficScore,
            nearestRoad: data.nearestRoad || null,
          });
        }
      } catch {
        // Non-blocking if API is unavailable
      }
    };
    loadTraffic();
    return () => {
  ...
  const displayImpressions =
    trafficEstimate?.estimatedDailyImpressions ||
    bb.estimatedImpressions ||
    bb.impressions;
  const trafficScore = trafficEstimate?.trafficScore ?? bb.trafficScore;
  const nearestRoad = trafficEstimate?.nearestRoad || bb.nearestRoad || null;
  const safeImpressions = Math.max(1, displayImpressions);
  const totalImpressions = Math.round(safeImpressions * roiDays);
  const reach = Math.round(totalImpressions * 0.35);
  const cpm = ((roiBudget / totalImpressions) * 1000).toFixed(2);
  const brand_recall = Math.min(
    99,
    Math.round(40 + roiDays * 0.8 + bb.scores.visibility * 0.3),
  );

  const maxHourly = Math.max(...bb.traffic.hourly);
  const peers = allBillboards
    .filter((b) => b.id !== bb.id && b.type === bb.type)
    .slice(0, 3);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/30 to-slate-50">
      ...
      <div className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200/60 px-4 py-3 flex items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          <span
            className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${avail.bg} ${avail.text}`}
          >
            <span
              className={`w-1.5 h-1.5 rounded-full ${avail.dot} animate-pulse`}
            />
            {avail.label}
          </span>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid lg:grid-cols-12 gap-6">
          <div className="lg:col-span-7 space-y-4">
            <div className="rounded-2xl overflow-hidden relative h-72 lg:h-[420px] shadow-xl">
              <img
                src={bb.image}
                alt={bb.title}
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                ...
                <div className="rounded-xl bg-slate-50 p-3">
                  ...
                  <p className="text-sm font-semibold text-slate-800 truncate">
                    {nearestRoad ? nearestRoad.replace(/_/g, " ") : "N/A"}
                  </p>
                </div>
                <div className="rounded-xl bg-slate-50 p-3">
                  <p className="text-[10px] uppercase tracking-wider text-slate-400">
                ...
                <div className="rounded-xl bg-slate-50 p-3">
                  ...
                  <p className="text-sm font-semibold text-slate-800">
                    {trafficScore !== undefined ? trafficScore : "N/A"}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 lg:sticky lg:top-24">
              ...
              {(trafficScore !== undefined || nearestRoad) && (
                <div className="mt-2 flex flex-wrap gap-2 text-xs">
                  {trafficScore !== undefined && (
                    <span
                      className={`px-2.5 py-1 rounded-full font-semibold ${trafficScore >= 70 ? "bg-emerald-50 text-emerald-700" : trafficScore >= 40 ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-700"}`}
                    >
                      Traffic Score {trafficScore}
                    </span>
                  )}
                  {nearestRoad && (
                    <span className="px-2.5 py-1 rounded-full bg-slate-50 text-slate-600 font-semibold">
                      {nearestRoad.replace(/_/g, " ")}
                    </span>
                  )}
                </div>
              )}
              <div className="grid grid-cols-3 gap-2 mt-3">
                {[
                  ["Weekly", `₹${(bb.price * 7 * 0.9).toLocaleString()}`],
                  ["Monthly", `₹${(bb.price * 30 * 0.8).toLocaleString()}`],
                  [
                    "CPM",
                    `₹${((bb.price / safeImpressions) * 1000).toFixed(1)}`,
                  ],
                ].map(([l, v]) => (
                  <div
                    key={l}
              ...
              <div
                className={`mt-3 flex items-center gap-2 text-xs font-medium rounded-xl px-3 py-2 ${bb.market.priceVsAvg < 0 ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}
              >
                <TrendingUp className="w-3.5 h-3.5" />
                {Math.abs(bb.market.priceVsAvg)}%{" "}
                {bb.market.priceVsAvg < 0 ? "below" : "above"} market average
              </div>
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 text-xs text-slate-500 mt-0.5">
                  ...
                  <span>·</span>
                  <span>{bb.vendor.totalListings} listings</span>
                  <span>·</span>
                  <span>Since {bb.vendor.since}</span>
                </div>
              </div>
              <div className="text-right text-xs text-slate-500">
                <Clock className="w-3 h-3 inline mr-1" />
                {bb.vendor.responseTime}
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
              ...
              <div className="grid grid-cols-5 gap-2">
                {[
                  ...
                  {
                    l: "Illumination",
                    v: bb.scores.illumination,
                    c: "#f59e0b",
                  },
                  { l: "Angle", v: bb.scores.angle, c: "#10b981" },
                ].map((s) => (
                  <ScoreRing key={s.l} value={s.v} label={s.l} color={s.c} />
                ))}
              </div>
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className="bg-indigo-50 rounded-xl p-3 text-center">
                  <div className="text-2xl font-extrabold text-indigo-600">
                    {Math.round(
                      Object.values(bb.scores).reduce((a, b) => a + b, 0) / 5,
                    )}
                  </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                },
                {
                  ...
                  label: "Campaigns Run",
                  value: bb.campaigns.totalRuns,
                  sub: "total all-time",
                },
                {
                  icon: <Users className="w-5 h-5 text-violet-500" />,
                  label: "Renewal Rate",
                  value: `${bb.campaigns.renewalRate}%`,
                  sub: "advertisers return",
                },
                {
                  icon: <Zap className="w-5 h-5 text-amber-500" />,
                  label: "Market Rank",
                  value: `#${bb.market.rank}`,
                  sub: `of ${bb.market.totalInArea} in area`,
                },
              ].map((s) => (
                <div
                  key={s.label}
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
              ...
              <div className="flex items-center gap-2 mb-3">
                <span className="text-xs bg-violet-100 text-violet-700 font-semibold px-2 py-0.5 rounded-full">
                  Income Zone: {bb.demographics.incomeZone}
                </span>
              </div>
          ...
          <div className="space-y-5">
            {(trafficScore !== undefined ||
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  {
                    l: "Estimated Daily",
                    v: displayImpressions.toLocaleString(),
                    icon: "📈",
                  },
                  { l: "Traffic Score", v: trafficScore ?? "N/A", icon: "🚦" },
                ].map((s) => (
                  <div
                    key={s.l}
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                {
                  l: "Weekday Avg",
                  v: bb.traffic.weekdayAvg.toLocaleString(),
                  icon: "📅",
                },
                {
                  l: "Weekend Avg",
                  v: bb.traffic.weekendAvg.toLocaleString(),
                  icon: "🏖️",
                },
              ].map((s) => (
                <div
          ...
          <div className="space-y-5">
            ...
            <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-2xl p-6 text-white shadow-xl">
              ...
              <div className="grid grid-cols-2 gap-4 mb-5">
                <div>
                  <label className="text-xs text-indigo-200 mb-1.5 block">
                    Budget (₹)
                  </label>
                  <input
                    type="number"
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={roiDays}
                    onChange={(e) => setRoiDays(+e.target.value)}
                    className="w-full bg-white/15 border border-white/20 rounded-xl px-3 py-2.5 text-white font-semibold text-sm outline-none focus:ring-2 focus:ring-white/40"
                  />
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                {[
                  {
                    l: "Total Impressions",
                    v: totalImpressions.toLocaleString(),
                  },
        ...
        {activeTab === "market" && (
          <div className="space-y-5">
              ...
              {[
                {
                  l: "Area Rank",
                  v: `#${bb.market.rank}`,
                  sub: `of ${bb.market.totalInArea}`,
                },
                {
                  l: "Percentile",
                  v: `${bb.market.percentile}th`,
                  sub: "top performers",
                },
                {
                  l: "vs Market Price",
                  v: `${bb.market.priceVsAvg > 0 ? "+" : ""}${bb.market.priceVsAvg}%`,
                ...
                >
                  <div
                    className={`text-2xl font-extrabold ${bb.market.priceVsAvg < 0 && s.l.includes("vs") ? "text-emerald-600" : "text-indigo-600"}`}
                  >
                    {s.v}
                  </div>
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
              <h2 className="font-bold text-slate-800 mb-4">
                Similar {bb.type} Billboards Nearby
              </h2>
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3 bg-indigo-50 rounded-xl p-3 border border-indigo-200">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-indigo-600">
                      {displayImpressions.toLocaleString()} impressions/day
                    </div>
                  </div>
                ...
                {peers.map((p, i) => (
                  <Link
                    ...
                    key={p.id}
                    className="flex items-center gap-3 rounded-xl p-3 border border-slate-100 hover:bg-slate-50 transition-colors"
                  >
                    <span className="w-6 h-6 rounded-full bg-slate-200 text-slate-600 text-xs flex items-center justify-center font-bold">
                      {i + 2}
                    </span>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-slate-400">
                        {(
                          p.estimatedImpressions || p.impressions
                        ).toLocaleString()}{" "}
                        impressions/day
                      </div>
                    </div>
